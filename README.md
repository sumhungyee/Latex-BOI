...
## Instructions
### Docker Guide
Define `.env` with `"``TELEBOT_TOKEN``"` as an environment variable key with the value being your telegram bot token. (Ensure that this is parsed as a string). Uncomment `RUN cp .env ./` in `Dockerfile` and run with `docker build -t <name> .`
